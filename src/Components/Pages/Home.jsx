...
//             { src: "https://i.imgur.com/VyswD2n.jpg", alt: "Brand 3" }
//           ].map((brand, index) => (
//             <Box 
//               key={index} 
//               flex={1} 
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchRandomProducts = async () => {
      try {
        ...
        setRandomProducts(shuffled.slice(0, 40));
      } catch (error) {
  ...
  return (
    <div>
      ...
      <Box px={[4, 6, 8, 12]} maxW="1800px" mx="auto">
        {[...Array(Math.ceil(randomProducts.length / 6))].map((_, rowIndex) => {
          const rowProducts = randomProducts.slice(rowIndex * 6, (rowIndex + 1) * 6);
          return (
            <Flex 
              key={rowIndex}
              direction="row" 
