...
const Shop = ({ categoryFilter, searchTerm }) => {
  ...
  let cartTotal = cartProducts.reduce((acc, p) => acc + Number(p.price), 0);
    ...
    toast({
      ...
      render: () => (
        <Flex color="white" border="4px solid white" p={"10px"} bgColor="green.400">
          <CheckCircleIcon w={30} h={30} />
          <Text size="lg" ml="15px">{`${modalProps.name} is now in your cart!!!`}</Text>
        </Flex>
      ),
    });
    dispatch(addToCart(data));
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (category === "Fruits & Veg" || categoryFilter === "Fruits & Veg") {
          productsQuery = query(
            ...
            where("category", "in", ["Fruits & Veg", "Fruits", "Fresh Produce"])
          );
        } else if (brand) {
          productsQuery = query(collection(db, "products"), where("brand", "==", brand));
        } else if (category || categoryFilter) {
          const activeCategory = category || categoryFilter;
          if (activeCategory === "Brands") {
            productsQuery = collection(db, "products");
          } else {
            productsQuery = query(collection(db, "products"), where("category", "==", activeCategory));
          }
        }
        ...
        // Apply price filter
        const filteredProducts = fetchedProducts.filter(p => parseFloat(p.price) <= price);

        // Remove duplicates based on id
        const uniqueProducts = Array.from(
          new Map(fetchedProducts.map(product => [product.id, product])).values()
        );
    ...
    fetchProducts();
  }, [brand, category, price, categoryFilter, toast]);

  return (
    <>
      <Flex w="90%" m="auto" justify="space-between" mt={["12", "12", "12", "12", "auto"]}>
        <Box w="25%" display={["none", "none", "none"]} style={{ fontFamily: "sans-serif" }} />
        <Box w={["100", "100%", "100%"]} mt="25px">
          <Heading mb="30px">{`Up to 50% off!`}</Heading>
          <Flex justify="space-between">
            <Select
              w="30%"
          ...
          {loading ? (
            <SimpleGrid w="100%" columns={[2, 2, 3]} spacing="40px" pt="10">
              {Array(10)
                .fill("")
                .map((_, i) => (
                  <Box key={i} padding="6" h="470px" boxShadow="md" bg="white">
                    <Skeleton h="200px" startColor="pink.100" mb="25px" endColor="orange.100" />
                    <Skeleton h="16px" w="100%" startColor="orange.300" endColor="pink.200" mb="15px" />
                    <Skeleton h="16px" w="85%" mb="15px" />
                    <Skeleton h="30px" mb="15px" w="55%" />
                    <Flex mb="15px">
                      {Array(5)
                        .fill("")
                        .map((_, j) => (
                          <SkeletonCircle key={j} endColor="yellow.100" startColor="gold" mr="5px" size="15px" />
                        ))}
                    </Flex>
                    <Skeleton h="20px" mb="15px" w="30%" />
                    <Skeleton h="30px" mb="15px" w="100%" endColor="gray.600" />
        ...
        <Modal isCentered onClose={onClose} isOpen={isOpen} motionPreset="slideInBottom" size={["sm", "md", "2xl"]}>
          <ModalOverlay />
          <ModalContent borderRadius="0px">
            <ModalHeader p="10px" bgColor="gainsboro" borderBottom="1px solid black" fontSize="20px">
              {`Get Your ${modalProps.category} Fast Order Is Limited!!!`}
            </ModalHeader>
            <ModalCloseButton />
            <ModalBody pt="35px" pb="35px">
              <Flex mt="10px" justify="space-between">
                ...
                <Box width="50%">
                  <Text mb="15px" fontSize="xl">{modalProps.name}</Text>
                  <Text mb="15px">{`Brand - ${modalProps.brand || 'Generic'}`}</Text>
                  <Text mb="15px">Quantity 1</Text>
                  <Heading>{`£${modalProps.price || 'Price not available'}`}</Heading>
                </Box>
              ...
              <Flex p="15px" mt="25px" borderTop="1px solid black" borderBottom="1px solid black" justify="space-between" direction={["column", "column", "row"]}>
                <Button
                  borderRadius="0px"
                  bgColor="white"
                  border="1px solid black"
                ...
                <Box w={["100%", "100%", "48%"]}>
                  <Link to="/cart">
                    <Button borderRadius="0px" bgColor="black" color="white" w="100%" display="block" _hover={{ bgColor: "#28bdb7" }}>
                      VIEW CART
                    </Button>
                  </Link>
                </Box>
